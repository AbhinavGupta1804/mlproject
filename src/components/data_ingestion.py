...
#The os module in Python provides functions to interact with the operating system, such as handling file paths
# , creating directories, reading environment variables, and executing system commands.
    ...
    def initiate_data_ingestion(self):  #to read data from database
        ...
        try:
            ...
            os.makedirs(os.path.dirname(self.ingestion_config.train_data_path),exist_ok=True) #If the directory doesn’t exist, os.makedirs() will create it. 
                         #this will fetch the name of directory frm filepath                  #If it already exists, the exist_ok=True argument prevents errors.
#Agar train, test, aur raw data same folder (artifacts/) me hain, toh kisi bhi file ka 
# os.path.dirname() use karne par same directory (artifacts) milegi. Ek baar os.makedirs() se 
# kisi bhi ek file ka folder ensure karna kaafi hai, kyunki sab ek hi jagah store ho rahe hain. ✅
...
#Agar raw data save nahi kiya, toh har baar data fetch karne par alag results        # specified by self.ingestion_config.raw_data_path, which is defined as artifacts/data.csv
#aa sakte hain (especially if source updates). ->WHY WE NEED RAW FILE 
            #abhi apan ne notebook se data uthaya # Us data ko artifacts/data.csv me store kar diya  
            logging.info("Train test split initiated")
            train_set,test_set=train_test_split(df,test_size=0.2,random_state=42)

            train_set.to_csv(self.ingestion_config.train_data_path,index=False,header=True)
            #dataset          #location for storing
...
if __name__=="__main__": #It makes sure the code inside this runs only when you run this file directly, not when you import it somewhere else.
    ...
    print(modeltrainer.initiate_model_trainer(train_arr,test_arr)) #prints r2 score of best model after applying best parameters
